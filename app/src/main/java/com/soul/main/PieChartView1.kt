 ...
 *
 */
class PieChartView1(context: Context, attrs: AttributeSet?, defStyleAttr: Int) :
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        outerRadius = min(measuredWidth / 2f, measuredHeight / 2f)
        X = measuredWidth / 2f
        Y = measuredHeight / 2f
        rectF?.apply {
            left = X - outerRadius
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        if (isRing(x, y)) {
            ...
            Log.d(
                TAG,
                "onTouchEvent: mPosition = ${mPosition}, position = $position, action = ${action}"
            )
