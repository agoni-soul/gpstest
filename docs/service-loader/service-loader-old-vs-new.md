...
> **定位**：文中的「改动方案」已落地为当前实现。完整总结、新流程图与对照表见
> [优化后设计与流程](service-loader-optimized-design.md)。  
> 本文保留条目对照底稿。

记录时间：2026-09-06

第 1 问描述的是**优化前实现**（固定 `ServiceInit_`、魔法默认 key、`getAll().size` 兜底）。  
本文对照模块化 Init、打包插件聚合、按 Class 去重的 `ServiceRecord`、更完整的 Helper。
