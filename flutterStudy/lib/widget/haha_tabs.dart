  ...
  Widget build(BuildContext context) {
    ...
    if (icon == null) {
      ...
      label = Column(
        ...
        children: <Widget>[
          Container(margin: const EdgeInsets.only(bottom: 10.0), child: icon),
          _buildLabelText(),
        ],
      );
    }
...
class _IndicatorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (controller.indexIsChanging) {
      ...
      final Rect? previous = currentIndex > 0
      ...
      if (value == index - 1.0) {
        ...
        _currentRect = next == null
            ? middle
            : Rect.lerp(middle, next, value - index);
      }
    }
...
class _DragAnimation extends Animation<double>
  ...
  double get value {
    ...
    return (controller!.animation!.value - index.toDouble()).abs().clamp(
      0.0,
      1.0,
    );
  }
      ...
      builder: (BuildContext context, Widget? child) {
        return Semantics(
          child: Row(
            ...
            children: List<Widget>.generate(tabController.length, (
              int tabIndex,
            ) {
