...
class HtmlUtils {
  static generateCode2HTml(String? mdData,
      {String backgroundColor = HahaColors.miWhiteString,
      String lang = 'java',
      userBR = true}) {
  ...
  static generateHtml(String? mdData,
      {String backgroundColor = HahaColors.webDraculaBackgroundColorString,
      userBR = true}) {
    if (mdData == null) {
      return "";
    }
    ...
    return generateCodeHtml(mdDataCode, false,
        ...
        actionColor: HahaColors.actionBlueString,
        userBR: userBR);
  }

  /// style for mdHTml
  static generateCodeHtml(mdHTML, wrap,
      {backgroundColor = HahaColors.white,
      String actionColor = HahaColors.actionBlueString,
      userBR = true}) {
    ...
    return "${"${"<html>\n" + "<head>\n" + "<meta charset=\"utf-8\" />\n" + "<title></title>\n" + "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0\"/>" + "<meta name=\“app-mobile-web-app-capable\”  content=\“yes\" /> " + "<link rel=\"stylesheet\" href=\"https://cdnjs.cloudflare.com/ajax/libs/highlight.js/9.12.0/styles/default.min.css\">\n" + "<script src=\"https://cdnjs.cloudflare.com/ajax/libs/highlight.js/9.12.0/highlight.min.js\"></script>" + "<script src=\"https://cdnjs.cloudflare.com/ajax/libs/highlight.js/9.12.0/languages/go.min.js\"></script>" + "<script>hljs.configure({'useBR': ${userBR.toString()}"
        "});hljs.initHighlightingOnLoad();</script> " + "<style>" + "body{background: $backgroundColor"};}a {color:$actionColor !important;}.highlight pre, pre { word-wrap: ${wrap ? "break-word" : "normal"};  white-space: ${wrap ? "pre-wrap" : "pre"}; }thead, tr {background:${HahaColors.miWhiteString};}td, th {padding: 5px 10px;font-size: 12px;direction:hor}.highlight {overflow: scroll; background: ${HahaColors.miWhiteString}}tr:nth-child(even) {background:${HahaColors.primaryLightValueString};color:${HahaColors.miWhiteString};}tr:nth-child(odd) {background: ${HahaColors.miWhiteString};color:${HahaColors.primaryLightValueString};}th {font-size: 14px;color:${HahaColors.miWhiteString};background:${HahaColors.primaryLightValueString};}</style></head>\n<body>\n" + mdHTML}</body>\n</html>";
  }
  ...
  static resolveHtmlFile(var res, String defaultLang) {
    if (res != null && res.result) {
      ...
      if ('markdown' == lang || 'md' == lang) {
        return generateHtml(res.data, backgroundColor: HahaColors.miWhiteString);
      } else {
