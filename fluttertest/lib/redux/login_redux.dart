...
///中间过程处理
class LoginMiddleware implements MiddlewareClass<HahaState> {
  @override
  void call(Store<HahaState> store, dynamic action, NextDispatcher next) {
...
///中间过程处理
Stream<dynamic> loginEpic(Stream<dynamic> actions, EpicStore<HahaState> store) {
  Stream<dynamic> loginIn(
      LoginAction action, EpicStore<HahaState> store) async* {
...
///中间过程处理
Stream<dynamic> oauthEpic(Stream<dynamic> actions, EpicStore<HahaState> store) {
  Stream<dynamic> loginIn(
      OAuthAction action, EpicStore<HahaState> store) async* {
    CommonUtils.showLoadingDialog(action.context);
