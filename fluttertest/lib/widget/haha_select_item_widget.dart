...
class HahaSelectItemWidget extends StatefulWidget
  ...
  @override
  _HahaSelectItemWidgetState createState() => _HahaSelectItemWidgetState();
...
class _HahaSelectItemWidgetState extends State<HahaSelectItemWidget> {
  int selectIndex = 0;
  int preSelIndex = 0;
  List keys = [false,false];

  _HahaSelectItemWidgetState();
  ...
  _renderItem(String name, int index) {
    var style = index == selectIndex
        ? HahaConstant.middleTextWhite
        : HahaConstant.middleSubLightText;
  ...
  _renderList() {
    ...
    for (int i = 0; i < widget.itemNames.length; i++) {
      if (i == widget.itemNames.length - 1) {
        ...
        list.add(_renderItem(widget.itemNames[i], i));
        list.add(Container(
            width: 1.0, height: 25.0, color: HahaColors.subLightTextColor));
      }
    }
    return list;
  }
