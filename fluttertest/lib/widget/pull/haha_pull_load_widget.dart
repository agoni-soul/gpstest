...
///通用下上刷新控件
class HahaPullLoadWidget extends StatefulWidget {
  ///item渲染
  ...
  ///控制器，比如数据和一些配置
  final HahaPullLoadWidgetControl? control;

  final Key? refreshKey;

  const HahaPullLoadWidget(
      this.control, this.itemBuilder, this.onRefresh, this.onLoadMore,
      {super.key, this.refreshKey, ScrollController? scrollController, bool? userIos});

  @override
  _HahaPullLoadWidgetState createState() => _HahaPullLoadWidgetState();
}

class _HahaPullLoadWidgetState extends State<HahaPullLoadWidget> {
    ...
    return SizedBox(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Text(context.l10n.app_empty, style: HahaConstant.normalText),
        ],
      ),
    );
  }
...
class HahaPullLoadWidgetControl {
  ///数据，对齐增减，不能替换
  List dataList = [];

  void addList(List extraList) {
